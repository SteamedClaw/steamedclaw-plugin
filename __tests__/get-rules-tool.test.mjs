...
/**
 * Tier 1 — mocked-runtime tests for the Path 3 plugin's get_rules tool.
 *
 * The tool lands a GET on /api/games/:gameId/rules via the plugin's
...
describe('get_rules tool — response-handling matrix', () => {
  it('registers alongside queue_match, get_turn, take_turn, get_strategy', async () => {
  ...
  it('returns not_registered when credentials.md is missing', async () => {
    ...
    const { api, captured } = makeMockApi();
    entry.register(api);
    const { payload } = await invokeGetRules(captured, 'tic-tac-toe');
  ...
  it('GETs /api/games/:gameId/rules with auth header', async () => {
    seedCreds();
    setResponse(() => ({
      status: 200,
      data: { gameId: 'tic-tac-toe', version: 'abc123', content: '# TTT rules' },
    }));
    const { default: entry } = await import('../index.js');
    const { api, captured } = makeMockApi();
    entry.register(api);
    await invokeGetRules(captured, 'tic-tac-toe');
    expect(mocks.http.calls.length).toBe(1);
    const call = mocks.http.calls[0];
    expect(call.opts.method).toBe('GET');
    expect(call.opts.path).toBe('/api/games/tic-tac-toe/rules');
  ...
  it('passes through {gameId, version, content} on HTTP 200', async () => {
    ...
    setResponse(() => ({
      status: 200,
      data: { gameId: 'nim', version: 'v1', content: '# Nim rules\n- take objects' },
    }));
    const { default: entry } = await import('../index.js');
    const { api, captured } = makeMockApi();
    entry.register(api);
    const { payload } = await invokeGetRules(captured, 'nim');
  ...
  it('url-encodes unusual gameIds so the path stays valid', async () => {
    ...
    const { api, captured } = makeMockApi();
    entry.register(api);
    await invokeGetRules(captured, 'weird id/with spaces');
    const call = mocks.http.calls[0];
    expect(call.opts.path).toBe('/api/games/weird%20id%2Fwith%20spaces/rules');
  });
  ...
  it('returns game_not_found on HTTP 404', async () => {
    ...
    const { api, captured } = makeMockApi();
    entry.register(api);
    const { payload } = await invokeGetRules(captured, 'mystery-game');
